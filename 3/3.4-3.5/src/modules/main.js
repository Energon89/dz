...
function addButtonClicked() {
  ...
  let colorAdd = checkIfColorCanBeAdded(color);
  if (colorAdd === false) {
    alert("There's no such a color");
  } else {
    colorAdd = checkIfColorAdded(color);
    if (colorAdd === false) {
      addColor(color);
      chooseColor(color);
      setColor(color);
...
function removeSelectedColorClicked() {
  ...
  let removeSelect = getSelectedValue();
  if (removeSelect === "not selected") {
    alert("Please choose a value to remove");
  } else {
    removeColor(removeSelect);
    reset();
  }
}
...
function addColor(color) {
  ...
  let optionElement = new Option(color);
